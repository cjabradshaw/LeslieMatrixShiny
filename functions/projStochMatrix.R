...
projStochMatrix <- function (x, initN, projYrs, projGens, iter, S_SD, F_SD, Qthresh=50, DFparams = c(DFa, DFb, DFc), CatParams = c(CMag, CMagSD))
{	
    ...
    if (missing(projYrs)) {
      prjyrs <- round(projGens * Gval(x,dimL), 0)
    }
    
    ## initial population vector
    initvec <- StableStageDist(x) * initN #initial population vector
    
    ## for founding populations (i.e., when no values in initvec >= 1), randomly allocate initial females to adult age classes
    if (length(which(initvec >= 1)) == 0) {
      ad.sub <- which(x[1,] > 0)
      alpha <- ad.sub[1]
      omega <- ad.sub[length(ad.sub)]
      initvec <- rep(0, dim(x)[1])
      initvec[sample(alpha:omega, initN, replace=T)] <- 1
      initvec
    }
    ...
    yrNow <- 1
    #************************
    yrEnd <- yrNow + (prjyrs) # set projection end date
    #************************
    t <- (yrEnd - yrNow)
