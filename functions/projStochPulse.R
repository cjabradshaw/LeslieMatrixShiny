...
#       TimeOff = timing (year) of offtake

projStochPulse <- function (x, initN, projYrs, projGens, iter, S_SD, F_SD, Qthresh=50, DFparams = c(DFa, DFb, DFc), CatParams = c(CMag, CMagSD),
                            PercOff=50, FixOff=10, TimeOff=5)
{	
    ## matrix dimension
    dimL <- dim(x)[1]
    ...
    if (missing(DFparams) & missing(CatParams)) {
      ...
      for (e in 1:iter) {
        ...
        ## single-pulse offtake parameters
        if(missing(TimeOff)) {
          yearOfft <- sample(1:prjyrs, 1)
        } else {
          yearOfft <- TimeOff
        }
